...
test("canvas accepts private and public material drag payloads", async () => {
    ...
    assert.match(source, /metadata: \{ \.\.\.imageMetadata\(image\), assetId: asset\.id, publicImageId: publicImageId \|\| undefined, mediaExpiresAt:/);
    ...
    assert.match(source, /uploadUserImage\(file, "canvas"\)/);
});
