...
func TestFetchDirectoryUsersAcceptsRolesWithoutDepartments(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"users":[{"userUid":"f3dbfc1a-06c5-4d31-a0cc-62e9475e34f1","displayName":"接收成员","enabled":true,"roles":["设计部"]}]}`))
	}))
	defer server.Close()
	previous := config.Cfg
	...
	if err != nil || string(encoded) != `{"userUid":"f3dbfc1a-06c5-4d31-a0cc-62e9475e34f1","displayName":"接收成员","enabled":true,"roles":["设计部"]}` {
